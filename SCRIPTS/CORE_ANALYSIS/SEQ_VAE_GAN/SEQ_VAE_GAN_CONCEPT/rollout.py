...
class ROLLOUT(object):
    def __init__(self, lstm, update_rate):
        ...
        with tf.device("/cpu:0"):
            self.processed_x = tf.transpose(tf.nn.embedding_lookup(self.g_embeddings, self.x),
                                            perm=[1, 0, 5])  # seq_length x batch_size x emb_dim
        ...
        # Initial states
        self.h0 = self.g_z_to_state_unit(self.z)
    ...
    def create_z_to_state_unit(self):
        self.Wz = tf.identity(self.lstm.Wz)  # z * hidden_dim
        self.bz = tf.identity(self.lstm.bz)  # hidden_dim
    ...
    def update_z_to_state_unit(self):
        self.Wz = self.update_rate * self.Wz + (1 - self.update_rate) * tf.identity(self.lstm.Wz)
        self.bz = self.update_rate * self.bz + (1 - self.update_rate) * tf.identity(self.lstm.bz)
    ...
    def update_params(self):
        ...
        self.g_z_to_state_unit = self.update_z_to_state_unit()
